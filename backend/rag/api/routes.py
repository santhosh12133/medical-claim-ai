...
from rag.schemas.policy import PolicyDocumentRead, PolicyIngestionResponse, PolicyMetadata, PolicyStatusResponse
...
async def ingest_policy(
    ...
    try:
        metadata = PolicyMetadata(
            title=title.strip(), policy_type=policy_type.strip(), policy_version=policy_version.strip(),
            department=department.strip(), effective_from=effective_from, effective_to=effective_to,
        )
    ...
    try:
        ...
        return PolicyIngestionResponse(
            policy_document_id=result.document_id, title=result.title, status=result.status,
            chunk_count=result.chunk_count, collection_name=settings.rag_collection_name,
...
def verify_claim(request: ClaimVerificationRequest, current_user: User = Depends(require_admin),
...
def verify_stored_claim(claim_id: int, current_user: User = Depends(require_admin),
                       verification_service=Depends(get_claim_verification_service)):
    try:
...
def list_policies(status_filter: str | None = None, current_user: User = Depends(require_admin),
...
def get_policy(policy_document_id: int, current_user: User = Depends(require_admin),
               policy_repository=Depends(get_policy_repository_service)):
...
def set_policy_status(policy_document_id: int, new_status: str = Form(...),
                      current_user: User = Depends(require_admin),
                      policy_repository=Depends(get_policy_repository_service),
    ...
    if policy.status == new_status:
        return PolicyStatusResponse(policy_document_id=policy.id, status=policy.status, message="Policy status unchanged")
    ...
    return PolicyStatusResponse(policy_document_id=policy.id, status=new_status, message=f"Policy marked {new_status}")


@router.get("/verifications", response_model=list[VerificationAuditRead])
def list_verifications(current_user: User = Depends(require_admin), verification_repository=Depends(get_verification_repository)):
