    ...
    def ingest(self, pdf_path: Path, original_filename: str, metadata: PolicyMetadata) -> IngestionResult:
        ...
        try:
            ...
            document = PolicyDocument(
                ...
                status="active",
                raw_text=raw_text,
                metadata_json=normalized_metadata.model_dump(),
            )
            ...
            self.logger.info("Indexed policy document %s with %s chunks", document.id, len(chunk_rows))
            ...
            if vector_ids:
                try:
                    ...
                    self.logger.exception("Failed to rollback vector store entries for a rejected ingestion")
